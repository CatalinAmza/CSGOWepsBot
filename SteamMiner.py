...
class SteamMiner(Thread):
    ...
    def get_inventory(self, steam_id):
        ...
        if inventory == -1:
            ...
            try:
                ...
                print("ERROR: " + str(Public_contents))
                return -1
            try:
                API_contents = json.loads(API_contents)
            except:
                print("ERROR " + str(API_contents))
                return -1
    ...
    def run(self):
        ...
        for item_name in self.collector.keys():
            ...
            if extract_wear_to_int(item_name) == limits[0] or (("rimson" in item_name or "erpent" in item_name or "owl" in item_name or "azard" in item_name or "nitro" in item_name.lower() or
                                                                    "whiteout" in item_name.lower() or "hyper beast" in item_name.lower()) and extract_wear_to_int(item_name) != limits[1]):
                ...
                if "oppler" in item_name or "owl" in item_name  or "ooth" in item_name or "blaze" in  item_name.lower():
